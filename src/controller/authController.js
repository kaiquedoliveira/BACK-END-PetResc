 ...
 try {
  ...
  const novaConta = await prisma.account.create({
   data: {
    ...
    ...(role === 'ONG' && { ong: { create: { name: nome, cnpj, descricao, endereco } } }), 
        // --- FIM DA CORREÇÃO ---

    // 'publico' espera 'nome' (com 'e')
    ...(role === 'PUBLICO' && { publico: { create: { nome: nome, cpf: cpf } } }),
   },
...
exports.login = async (req, res) => {
 ...
 try {
  ...
  const nomeDoUsuario = usuario.publico?.nome || usuario.ong?.name || usuario.admin?.nome || 'Usuário';
