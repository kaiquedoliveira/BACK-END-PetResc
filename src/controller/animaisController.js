    ...
    const animais = await prisma.animal.findMany({
      where: {
        AND: [
          ...
          // AQUI ESTÁ A CORREÇÃO DO FEED (Traz Lar Temporário junto)
          status
...
const criarAnimal = async (req, res) => {
  ...
  const {
    ...
    vermifugado, data_vermifugado, vacinado, vacinas_texto,
    castrado, data_castrado, testado_doencas, testes_texto, resultados_testes,
    observacoes, cuidado, sociabilidade,
  } = req.body;
  ...
  try {
    const novoAnimal = await prisma.animal.create({
      data: {
        ...
        testado_doencas: parseBool(testado_doencas),
        ...
        ficha: {
          create: {
            ...
            castrado: parseBool(castrado),
            observacoes: observacoes || cuidado || null,
            nome: nome,
            especie: especie,
          },
        },
...
const atualizarAnimal = async (req, res) => {
  ...
  const {
    nome, idade, data_resgate, data_vermifugado, dataCastrado,
    vermifugado, vacinado, castrado, testado, tinha_filhotes, tinha_coleira,
    ...dataRestante
  } = req.body;
